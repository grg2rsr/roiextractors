...
class ScanImageImagingExtractor(ImagingExtractor):
    ...
    def _read_ifd_as_frame(self, page) -> np.ndarray:
        """Read a single IFD page into a 2D frame array.

        This is the seam used by `get_series` to materialize one page. The base implementation
        returns the full page; subclasses (e.g. the multi-ROI extractor) override it to crop a
        sub-region out of each page.

        Parameters
        ----------
        page : tifffile.TiffPage
            The IFD page to read.

        Returns
        -------
        numpy.ndarray
            The 2D frame array with shape (num_rows, num_columns).
        """
        return page.asarray()
    ...
    def get_series(self, start_sample: int | None = None, end_sample: int | None = None) -> np.ndarray:
        ...
        for return_index, sample_index in enumerate(range(start_sample, end_sample)):
            for depth_position in range(num_planes):
                ...
                samples[return_index, :, :, depth_position] = self._read_ifd_as_frame(image_file_directory)
...
class ScanImageMultiROIImagingExtractor(ScanImageImagingExtractor):
    ...
    selection, and frame-to-IFD mapping; it only adds ROI geometry parsing and per-page cropping.
    ...
    def __init__(
        ...
        # Present this single ROI as the frame by replacing the page dimensions with the ROI's.
    ...
    def _read_ifd_as_frame(self, page) -> np.ndarray:
        """Crop the selected ROI's rows out of a stacked multi-ROI page.

        Parameters
        ----------
        page : tifffile.TiffPage
            The IFD page holding all stacked ROIs.

        Returns
        -------
        numpy.ndarray
            The selected ROI's frame with shape (roi_num_rows, roi_num_columns).
        """
        return page.asarray()[self._roi_row_start : self._roi_row_end, :]
